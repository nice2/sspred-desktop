...
def get(seq):
	...
	while message == '' or time.time() > stime +  900: #loops until desired email is found or 15 min elapse
		print("Sable Not Ready ")
		time.sleep(30)
		for e in session.get_email_list():			#For each email in inbox
			...
			if data is not None:					#Checks if email body is empty
				for dline in data.splitlines():		#Splits body into lines
					if "Query:" in dline:			#Checks if Query: line exists
						if dline[6:].strip() == randName:	#Checks if query is same as inputed seq name
							message = html.unescape(data)	#Sets message variable to email contents
							email_id = True					#Allows next part to run
